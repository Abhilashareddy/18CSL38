...
void attach(int coef, int x, int y, int z, NODE *dest){
	NODE temp;
	temp = getnode();
	temp -> coef = coef;
	temp -> x = x;
...
NODE add(NODE a,NODE b){
	...
	do{
		switch(compare(a,b)){
			case -1:
				attach(b->coef, b->x, b->y, b->z, &prev_ans);
				// goto next node
				b = b->link;
				break;
			case 0:
				...
				sum = a->coef+b->coef;
				if(sum)
					attach(sum, a->x, a->y, a->z, &prev_ans);
				// goto next nodes
				a = a->link;
				b = b->link;
				}
				break;
			case 1:
				attach(a->coef,a->x,a->y,a->z, &prev_ans);
